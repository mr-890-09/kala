...
if ($db) {
    try {
        $stmt = $db->prepare("SELECT * FROM user_preferences WHERE session_id = :session_id");
...
<head>
    ...
    <script>
        // User preferences from PHP/SQLite
        const userPreferences = {
            sessionId: '<?php echo session_id(); ?>',
            temperatureUnit: '<?php echo $preferences['temperature_unit'] ?? 'C'; ?>',
            favorites: <?php echo $preferences ? json_encode(json_decode($preferences['favorite_locations'], true) ?? []) : '[]'; ?>
        };
        ...
        window.onload = function() {
            ...
            console.log("FisherWeather rakendus on laaditud - SQLite andmebaas aktiiviine");
        }
            ...
            fetch('api.php?action=get_history')
                ...
                .then(data => {
                    if (data.history && data.history.length > 0) {
                        console.log('Search history loaded:', data.history);
                    }
                })
                .catch(error => console.log('History loading info:', error));
        }

        // Save weather data to SQLite
        function saveWeatherToDatabase(location, weatherData) {
            const formData = new FormData();
            formData.append('location', location);
            formData.append('temperature', weatherData.temperature || 0);
            formData.append('description', weatherData.description || '');
            formData.append('wind_speed', weatherData.windSpeed || 0);
            formData.append('wind_direction', weatherData.windDirection || '');
            formData.append('humidity', weatherData.humidity || 0);
            formData.append('pressure', weatherData.pressure || 0);
            formData.append('visibility', weatherData.visibility || 0);
            formData.append('cloud_cover', weatherData.cloudCover || 0);
            formData.append('raw_data', JSON.stringify(weatherData));

            fetch('api.php?action=save_weather', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => console.log('Weather saved to database:', data))
            .catch(error => console.log('Database save info:', error));
        }
        ...
        function addToHistory(location) {
            ...
            fetch('api.php?action=add_to_history', {
                method: 'POST',
                body: formData
            })
            .catch(error => console.log('History tracking info:', error));
        }

        // Save user preferences
        function saveUserPreference(favorites, tempUnit) {
            const formData = new FormData();
            formData.append('favorites', JSON.stringify(favorites));
            formData.append('unit', tempUnit);

            fetch('api.php?action=save_preference', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .catch(error => console.log('Preference save info:', error));
        }
    </script>
